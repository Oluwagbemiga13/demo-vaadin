...
public class CreateAlert extends VerticalLayout {
    ...
    private Grid<JoinItemDTO> resultsGrid;  // Assuming your entities have a common base or use Object and customize columns later
    ...
    private void setupLayout() {
        ...
        searchOptions.addValueChangeListener(event ->{
            ...
            if ("Organ Name".equals(selectedValue)) {
                ...
                // Do something specific when Organ Name is selected
        ...
        searchBox.setItemLabelGenerator(item -> {
            if (item instanceof OrganDTO) {
                return item.getName();
            } else if (item instanceof SymptomDTO) {
                return item.getName();
            } else if (item instanceof PartDTO) {
                return item.getName();
            } else {
                return "";  // default, shouldn't really reach here
            }
        });

//        searchBox.addValueChangeListener(event -> {
//           resultsGrid.setItems(symptomPartService.getAll());
//        });

        searchButton.addClickListener(event -> {
            resultsGrid = gridManager.createLonelyGrid(symptomPartService,new String[] {"name"});
            log.info("Size: " + String.valueOf(symptomPartService.getAll().size()));
            add(resultsGrid);
        });
    ...
    private void search() {
        ...
        if ("Organ Name".equals(selectedOption)) {
            // Call service method for organ search and update the resultsGrid
        } else if ("Symptom Name".equals(selectedOption)) {
            // Call service method for symptom search and update the resultsGrid
        } else if ("Body Part Name".equals(selectedOption)) {
            // Call service method for body part search and update the resultsGrid
        }
    }
